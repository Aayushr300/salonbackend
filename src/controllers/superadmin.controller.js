...
const {
  ...
  getSuperAdminTopSellingItemsDB,
  getSuperAdminSalesVolumeDB,
  getSuperAdminOrdersProcessedDB,
  getTenantsDB,
  addTenantDB,
  updateTenantDB,
  getTenantCntByIdDB,
  getTenantDetailsByIdDB,
  logoutAllUsersOfTenantDB,
  deleteTenantDB,
  getTenantsDataByStatusDB,
  getTenantSubscriptionHistoryDB,
  getTenantTotalUsersDB,
  getTenantDetailsDB,
  getTenantStoreDetailsDB,
...
exports.getTenants = async (req, res) => {
  try {
    ...
    const result = await getTenantsDB(
      page,
      perPage,
      search,
      status,
...
exports.getSuperAdminDashboardData = async (req, res) => {
  try {
    const [activeTenants, ordersProcessedToday, salesVolumeToday, mrr, arr] =
      await Promise.all([
        getActiveTenantsDB(),
        getOrdersProcessedTodayDB(),
        getSalesVolumeTodayDB(),
...
exports.addTenant = async (req, res) => {
  try {
    ...
    const tenantData = await addTenantDB({
      name,
      email,
      password,
      isAdmin,
    ...
    return res
      .status(200)
      .json({ message: "Tenant added successfully", tenant: tenantData });
  } catch (error) {
    ...
    return res
      .status(500)
      .json({ message: "An error occurred while adding the tenant" });
  }
};
...
exports.updateTenant = async (req, res) => {
  try {
    ...
    if (
      currentTenant.username !== email ||
      (isActive == 0 && currentTenant.is_active == 1)
    ) {
    ...
    return res
      .status(500)
      .json({ message: "An error occurred while updating the tenant" });
  }
};
