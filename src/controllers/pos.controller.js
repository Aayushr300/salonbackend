...
exports.createOrder = async (req, res) => {
  try {
    ...
    console.log("Create Order Discount", req.body, discount, payableAmount);
...
exports.createOrderAndInvoice = async (req, res) => {
  try {
    ...
    // Calculate the correct total (finalTotal)
    const finalTotal =
      Number(netTotal) + Number(taxTotal) - Number(discount || 0);
...
exports.getPOSQROrders = async (req, res) => {
  try {
    ...
    const { kitchenOrders, kitchenOrdersItems, addons } =
      await getPOSQROrdersDB(tenantId);
