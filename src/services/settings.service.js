...
// Get all coupons
exports.getCouponsDB = async (tenantId) => {
  const conn = await getMySqlPromiseConnection();
  try {
    const sql = `SELECT * FROM coupons WHERE tenant_id = ?;`;
    const [result] = await conn.query(sql, [tenantId]);
    return result;
  } finally {
    conn.release();
...
// Add a coupon
exports.addCouponDB = async (code, discount, expiry, tenantId) => {
  const conn = await getMySqlPromiseConnection();
  try {
    const sql = `INSERT INTO coupons (code, discount, expiry, tenant_id) VALUES (?, ?, ?, ?)`;
    await conn.query(sql, [code, discount, expiry, tenantId]);
  } finally {
    conn.release();
  }
};
...
// Update a coupon
exports.updateCouponDB = async (id, code, discount, expiry, tenantId) => {
  const conn = await getMySqlPromiseConnection();
  try {
    const sql = `UPDATE coupons SET code=?, discount=?, expiry=? WHERE id=? AND tenant_id=?`;
    await conn.query(sql, [code, discount, expiry, id, tenantId]);
  } finally {
    conn.release();
  }
};
...
// Delete a coupon
exports.deleteCouponDB = async (id, tenantId) => {
  const conn = await getMySqlPromiseConnection();
  try {
    const sql = `DELETE FROM coupons WHERE id=? AND tenant_id=?`;
    await conn.query(sql, [id, tenantId]);
  } finally {
    conn.release();
  }
