...
exports.getSusUserDB = async (tenantId) => {
  ...
  try {
    ...
    const sql = `
        SELECT username,  role, phone, email, scope  FROM users 
        WHERE tenant_id = ?;
        `;
...
exports.updateTenantSubscriptionAccess = async (
  ...
  try {
    const sql = `
        UPDATE tenants
        SET is_active = ?, subscription_id = ?, payment_customer_id = ?, subscription_start = ?, subscription_end = ? , subscription_duration_months = ? , subscription_price = ? , subscription_duration_text = ?
        WHERE id = (
